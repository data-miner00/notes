...
대답이 "아마도"인 경우 구선된 허용된 오탐룰을 기반으로 데이터가 실제로 거기에 있을 가능성이 높습니다. 대답이 "아니요"이면 데이터가 세트에 확실히 존재하지 않은 것입니다. 즉, 플룸 필터는 거짓 긍정(유형 I 오류)을 얻을 가능성이 _낮은_ 확률을 가질 수 있지만 거짓 부정은 _절대_ 없습니다.
