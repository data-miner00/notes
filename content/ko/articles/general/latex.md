...
동일한 이미지 유형이 여러 개인 경우 컴파일러가 가장 적합한 확장자를 찾을 수 있도록 이미지 파일의 확장자를 지정할 필요가 없습니다.
...
LaTeX에는 두 가지 유형의 분수가 있습니다. 표시 스타일 및 텍스트 스타일. 디스플레이 스타일은 디스플레이 블록에 표시되고 텍스트 스타일은 인라인 환경 내에서 사용됩니다. LaTeX는 환경에 따라 자동으로 최상의 스타일을 선택하지만 이를 명시적으로 지정할 수 있는 옵션도 있습니다. 표시 스타일 분수에는 `\dfrac`을, 텍스트 스타일 분수에는 `\tfrac`을 사용할 수 있습니다.
